...
>
> **▶ STAGE 3 EXECUTION ORDER (derived 2026-06-26 — verified by grep; do in ONE atomic green commit):**
> Callers verified: `loadWithRegistry`/`Registry.build`/`store.load()` have ZERO production callers — only `startTestServer` (helpers.ts) + `auth.route-gating` (3 sites). So:
> 1. **REWRITE `auth.route-gating.e2e` FIRST** (off the legacy `POST /content-types` + `loadWithRegistry`). DESIGN: replace the manual auth wiring with `startTestServerFromFilesWithAuth(countedSql, emptyGenDir)` (the S6 helper already wires real better-auth + entitiesDir ⇒ `builderActive` ⇒ the `/builder` routes register; it returns `signUp`/`userIdOf`/`grantRole`/`sessionCache`/`rbac`). Pass the COUNTED `postgres(url,{debug})` handle so the zero-PG test still counts. Create `dpgate` (draftPublish) + `crudt` via `PUT /builder/content-types/:apiId` AS the bootstrapped super-admin in setup. Then REPOINT the gate matrix: **builder.manage** gating (401/403/2xx) → `PUT /builder/content-types/:apiId` (was `POST /content-types`); **content.\*** gating → data writes on `crudt` (already #5); **media.upload** → `POST /_files/upload` (already #11). Drop the per-field/relation/component-field 401 loop + every `content_types` meta-row SELECT (replace with `ct_<type>` checks). Keep #2 path-norm (probe `//crudt`), #9 draft-leak, #6/#7/#8 bootstrap (pure RBAC). The fresh-db race test: swap its inline `loadWithRegistry()`→`loadFromSchemas([])`. Teardown: `sessionCache.stop()` + `closeAuth()`.
> 2. DELETE `startTestServer` from `helpers.ts` (3.7) — removes the other `loadWithRegistry` caller.
> 3. DELETE `loadWithRegistry` + `load()` from `PostgresStore` AND the `Store` interface (`src/store/store.ts`) (3.4).
> 4. DELETE `Registry.build`/`rebuildType`/`rebuildComponent` + the meta-read imports in `registry.ts` (3.3).
> 5. Gut `uws.adapter.ts` (3.1): imports `:18-19`; `handleContentTypeRoute`/`handleComponentTypeRoute`; `ctCtx`/`ctMutate` + the `/content-types*` registrations `:907-914` (incl. the legacy GET reads — auth.bridge's probe tolerates the resulting 404); `cmpCtx`/`cmpMutate` + `/component-types*` `:1150-1155`; the `builderActive` flag `:849` (the `/builder` routes register on their own `entitiesDir!==undefined` check — VERIFY they don't read `builderActive`).
> 6. DELETE both controller files (3.2). 7. Gut the two repos (3.5/3.6) — keep `resolveFields`/`rejectTopLevelRelation`/`resolveComponentFields` + the row TYPES; `tsc --noEmit` after each repo. 8. VERIFY green.
>
