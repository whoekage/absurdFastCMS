/**
 * node:test --test-global-setup hook for the @conti/sdk integration suite.
 *
 * The SDK is tested against a REAL @conti/api server backed by a REAL Testcontainers Postgres, so the
 ...
 *
 * @conti/api has no public `exports`, so the harness imports its internals by relative path
