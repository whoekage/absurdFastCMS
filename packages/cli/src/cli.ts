...
// One folder per entity (entities/<apiId>/). This schema.ts is owned/regenerated by the visual Builder;
...
// entities/article/hooks.ts — the OPTIONAL lifecycle file (one per entity that needs hooks). The visual
...
// entities/article/services.ts + controller.ts — custom per-entity logic (reusable functions + custom
...
export async function initProject(dir: string, opts: { name?: string } = {}): Promise<void> {
  ...
  // entities/<apiId>/ ships the demo `article` (one folder per content-type): schema + hooks + custom-logic
  // placeholders. This is the code-first source of truth.
  const articleDir = path.join(dir, 'entities', 'article');
