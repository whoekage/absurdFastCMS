...
interface BaseChange {
  ...
  /** The module's CURRENT (next) name — the table is `ct_<name>` (post-rename for a renamed type). */
...
export interface DropField extends BaseChange { readonly kind: 'dropField'; readonly fieldId: string; readonly fieldName: string; }
...
export interface RetypeField extends BaseChange { readonly kind: 'retypeField'; readonly fieldId: string; readonly fieldName: string; readonly from: ResolvedType; readonly to: ResolvedType; readonly classification: 'metadata-only' | 'rewrite' | 'forbidden'; }
/** Same id+name, changed nullability. → NOT NULL is data-dependent (existing nulls); → NULL is safe. */
export interface SetFieldNullable extends BaseChange { readonly kind: 'setFieldNullable'; readonly fieldId: string; readonly fieldName: string; readonly from: boolean; readonly to: boolean; }
...
function indexFields(schema: Schema): Map<string, FieldSchema> {
  ...
  for (const f of schema.fields) {
    if (m.has(f.id)) throw new SchemaDiffError(`module "${schema.name}" field "${f.name}": duplicate field id "${f.id}"`);
    m.set(f.id, f);
  }
  return m;
}
...
function indexRelations(schema: Schema): Map<string, RelationSchema> {
  ...
  for (const r of schema.relations ?? []) {
    if (m.has(r.id)) throw new SchemaDiffError(`module "${schema.name}" relation "${r.field}": duplicate relation id "${r.id}"`);
    m.set(r.id, r);
  }
  return m;
}
...
function addRelationChange(owner: Schema, rel: RelationSchema): AddRelation {
  const base: AddRelation = {
    kind: 'addRelation', typeId: owner.id, name: owner.name, risk: 'safe',
    relationId: rel.id, field: rel.field, relKind: rel.kind, target: rel.target, linkTable: deriveLinkTableName(owner.name, rel.field),
  };
...
function dropRelationChange(owner: Schema, rel: RelationSchema): DropRelation {
  return { kind: 'dropRelation', typeId: owner.id, name: owner.name, risk: 'destructive', relationId: rel.id, field: rel.field, linkTable: deriveLinkTableName(owner.name, rel.field) };
}
...
export function diff(prev: Schema[], next: Schema[]): ChangeSet {
  ...
  for (const [id, n] of nextTypes) {
    ...
    creates.push({ kind: 'addType', typeId: id, name: n.name, risk: 'safe', schema: n });
  ...
  for (const [id, p] of prevTypes) {
    ...
    drops.push({ kind: 'dropType', typeId: id, name: p.name, risk: 'destructive' });
  }
...
function diffMatchedType(typeId: string, p: Schema, n: Schema, alters: Change[], drops: Change[], relAdds: Change[], relDrops: Change[]): void {
  const name = n.name;
  // name rename → table rename (lossless). collectionName/info are presentation-only → no DDL.
  if (p.name !== n.name) {
    // Link-table names derive from the owner name, so renaming a type that owns relations would orphan
    // them. Deferred (loud) until link tables carry a stable name independent of the name.
    if ((p.relations?.length ?? 0) > 0 || (n.relations?.length ?? 0) > 0) {
      throw new SchemaDiffError(`renaming a module that owns relations ("${p.name}" -> "${n.name}") is deferred — link-table names derive from the name`);
    }
    alters.push({ kind: 'renameType', typeId, name, risk: 'safe', fromName: p.name, toName: n.name });
  }
  ...
  for (const [fid, pf] of pFields) {
    if (!nFields.has(fid)) alters.push({ kind: 'dropField', typeId, name, risk: 'destructive', fieldId: fid, fieldName: pf.name });
  }
  ...
  for (const [fid, nf] of nFields) {
    ...
    if (!pf) continue;
    if (pf.name !== nf.name) alters.push({ kind: 'renameField', typeId, name, risk: 'safe', fieldId: fid, from: pf.name, to: nf.name });
    ...
    if (!resolvedEqual(fromT, toT)) {
      ...
      alters.push({ kind: 'retypeField', typeId, name, risk: retypeRisk(classification), fieldId: fid, fieldName: nf.name, from: fromT, to: toT, classification });
    }
  ...
  for (const [fid, nf] of nFields) {
    ...
    alters.push({ kind: 'addField', typeId, name, risk, field: nf, sort: n.fields.findIndex((f) => f.id === fid) });
  }
  ...
  if (commonPrev.length === commonNext.length && commonPrev.some((id, i) => id !== commonNext[i])) {
    alters.push({ kind: 'reorderFields', typeId, name, risk: 'safe', order: n.fields.map((f) => f.id) });
  }
  ...
  for (const [rid, nr] of nextRels) {
    ...
    if (pr.field !== nr.field || pr.kind !== nr.kind || pr.target.toLowerCase() !== nr.target.toLowerCase() || (pr.inverseField ?? null) !== (nr.inverseField ?? null)) {
      throw new SchemaDiffError(`module "${n.name}" relation "${nr.field}": changing a relation (rename / kind / target / inverse) is deferred — drop and re-add it`);
    }
  }
