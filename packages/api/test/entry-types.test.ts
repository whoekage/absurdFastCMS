...
import { rawField, startTestServer, schema, closeAuth, type TestServer } from './helpers.ts';
...
after(async () => {
  if (server) server.close(server.token);
...
test('POST i64/decimal/json round-trips byte-identical to the GET; PUT changes them', async () => {
  const res = await server.fetch(`/metric`, {
    method: 'POST',
  ...
  const got = await (await server.anonFetch(`/metric/${id}`)).text();
  ...
  const put = await server.fetch(`/metric/${id}`, { method: 'PUT', body: JSON.stringify({ big: '42', amount: '0.05', payload: { nested: 'updated', k: [1, 2] } }) });
  assert.equal(put.status, 200);
  const putText = await put.text();
  assert.equal(rawField(putText, 'payload'), '{"k": [1, 2], "nested": "updated"}');
  const after = await (await server.anonFetch(`/metric/${id}`)).text();
...
test('invalid i64/decimal/json -> 400', async () => {
  ...
  for (const [label, payload] of Object.entries(cases)) {
    const res = await server.fetch(`/metric`, { method: 'POST', body: JSON.stringify(payload) });
