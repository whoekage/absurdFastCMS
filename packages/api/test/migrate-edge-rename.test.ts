...
test('rename + RETYPE the SAME field in one migrate: both ops, rename first, data cast preserved', async () => {
  ...
  const next = [ct('ct_a', 'thing', [f('f_v', 'view_count', 'biginteger', { nullable: true })])];
...
test('rename an ENUMERATION (status) field: CHECK follows the rename and still gates inserts', async () => {
  ...
  const r = await migrate(sql, [
    ct('ct_a', 'thing', [f('f_s', 'state', 'enumeration', { values: ['draft', 'published', 'archived'], nullable: false })]),
...
test('rename a NOT NULL field: stays NOT NULL, data intact, NULL insert still rejected', async () => {
  await migrate(sql, [ct('ct_a', 'thing', [f('f_n', 'name', 'string', { nullable: false })])]);
  await sql.unsafe(`INSERT INTO ct_thing (name) VALUES ('keep')`);

  const r = await migrate(sql, [ct('ct_a', 'thing', [f('f_n', 'label', 'string', { nullable: false })])]); // same id f_n
  assert.deepEqual(r.applied.map((c) => c.kind), ['renameField']);
...
test('SWAP two field names via ids in one migrate: blocked by a duplicate-column collision, tx rolls back', async () => {
  await migrate(sql, [
    ct('ct_a', 'thing', [
  ...
  // swap: f_a -> beta, f_b -> alpha. The diff emits two renameFields with NO temp-name staging, so applying
  // one rename collides with the still-present other column (42701 duplicate_column). The single tx aborts.
  const swapped = [
    ct('ct_a', 'thing', [
  ...
  await assert.rejects(migrate(sql, swapped)); // a Postgres duplicate-column error bubbles out of the tx

  // NOTHING changed: both original columns + the row are intact (atomic rollback).
  const cols = (await physicalColumns(sql, 'ct_thing')).map((c) => c.name).filter((n) => n === 'alpha' || n === 'beta');
  assert.deepEqual(cols.sort(), ['alpha', 'beta'], 'both original columns survive the rolled-back swap');
  const [row] = await sql<{ alpha: string; beta: string }[]>`SELECT alpha, beta FROM ct_thing`;
  assert.deepEqual({ a: row?.alpha, b: row?.beta }, { a: 'A', b: 'B' }, 'data unchanged after rollback');
});
...
test('rename a field to the OLD name of a DROPPED field (id reuse-free): rename succeeds, data preserved', async () => {
  ...
  const next = [ct('ct_a', 'thing', [f('f_keep', 'legacy', 'string', { nullable: true })])]; // f_old dropped, f_keep -> legacy
...
test('rename TYPE then rename it BACK: round-trips losslessly, the same rows survive both hops', async () => {
  ...
  await migrate(sql, [ct('ct_a', 'gadget', [f('f_t', 'title', 'string', { nullable: true })])]); // thing -> gadget
  assert.equal(await tableExists(sql, 'ct_gadget'), true);

  const r = await migrate(sql, [ct('ct_a', 'thing', [f('f_t', 'title', 'string', { nullable: true })])]); // gadget -> thing
  assert.deepEqual(r.applied.map((c) => c.kind), ['renameType']);
...
test('rename FIELD then rename it BACK: data survives the full round-trip', async () => {
  ...
  await migrate(sql, [ct('ct_a', 'thing', [f('f_t', 'headline', 'string', { nullable: true })])]); // title -> headline
  const r = await migrate(sql, [ct('ct_a', 'thing', [f('f_t', 'title', 'string', { nullable: true })])]); // headline -> title
  assert.deepEqual(r.applied.map((c) => c.kind), ['renameField']);
...
test('STABLE-id rename KEEPS data; a NEW id on a "renamed" field is drop+add => data LOST', async () => {
  ...
  const renamed = await migrate(sql, [ct('ct_a', 'thing', [f('f_t', 'headline', 'string', { nullable: true })])]);
  ...
  const reidentified = [ct('ct_a', 'thing', [f('f_x', 'headline', 'string', { nullable: true })])];
