...
test('manyToMany: set (bare + {set}), connect, disconnect; link rows + populated GET both directions', async () => {
  const schemas = [
    schema({ name: 'tag', fields: [{ name: 'label', type: 'string' }] }),
...
test('manyToOne: bare set, connect, disconnect; reassign on second connect (owner moves)', async () => {
  const schemas = [
    ...
    schema({ name: 'book', fields: [{ name: 'title', type: 'string' }], relations: [{ field: 'author', kind: 'manyToOne', target: 'author', inverseField: 'books' }] }),
  ];
...
test('oneToMany: connect a book to an author, then connect that book to another author REASSIGNS it', async () => {
  const schemas = [
    schema({ name: 'author', fields: [{ name: 'name', type: 'string' }], relations: [{ field: 'books', kind: 'oneToMany', target: 'book', inverseField: 'author' }] }),
    schema({ name: 'book', fields: [{ name: 'title', type: 'string' }] }),
  ];
  let a1!: number, a2!: number, b1!: number;
  await boot(schemas, async () => {
    a1 = await insertRow('ct_author', 'name', 'A1');
...
test('oneToOne: set then reassign on both sides (displaced partner unlinked)', async () => {
  const schemas = [
    schema({ name: 'person', fields: [{ name: 'name', type: 'string' }], relations: [{ field: 'passport', kind: 'oneToOne', target: 'passport', inverseField: 'holder' }] }),
    schema({ name: 'passport', fields: [{ name: 'code', type: 'string' }] }),
  ];
...
test('manyToMany: connect + disconnect together (disconnect-then-connect, connect wins overlap)', async () => {
  const schemas = [
    schema({ name: 'tag', fields: [{ name: 'label', type: 'string' }] }),
    schema({ name: 'book', fields: [{ name: 'title', type: 'string' }], relations: [{ field: 'tags', kind: 'manyToMany', target: 'tag' }] }),
  ];
  let t1!: number, t2!: number, t3!: number, b1!: number;
  await boot(schemas, async () => {
    t1 = await insertRow('ct_tag', 'label', 'a');
...
test('oneToOne self-referential: self-link is idempotent; reassign displaces the self-edge', async () => {
  const schemas = [
    schema({ name: 'node', fields: [{ name: 'name', type: 'string' }], relations: [{ field: 'spouse', kind: 'oneToOne', target: 'node', inverseField: 'spouseOf' }] }),
  ];
...
test('unknown relation field error message is "unknown field" (same as a scalar)', async () => {
  const schemas = [schema({ name: 'book', fields: [{ name: 'title', type: 'string' }] })];
  await boot(schemas, async () => {
...
test('FK: a non-existent related id -> 400, NO scalar change, NO link row', async () => {
  const schemas = [
    schema({ name: 'tag', fields: [{ name: 'label', type: 'string' }] }),
    schema({ name: 'book', fields: [{ name: 'title', type: 'string', options: { nullable: false } }], relations: [{ field: 'tags', kind: 'manyToMany', target: 'tag' }] }),
  ];
  let b1!: number;
  await boot(schemas, async () => {
...
test('self-referential manyToOne parent (+ inverse children): set + visible both directions', async () => {
  const schemas = [
    schema({ name: 'category', fields: [{ name: 'slug', type: 'string' }], relations: [{ field: 'parent', kind: 'manyToOne', target: 'category', inverseField: 'children' }] }),
  ];
  let c1!: number, c2!: number;
  await boot(schemas, async () => {
...
test('pure-scalar write is byte-identical: relation-less type, AND a type-with-relations omitting them', async () => {
  const schemas = [
    schema({ name: 'note', fields: [{ name: 'text', type: 'string', options: { nullable: false } }] }),
