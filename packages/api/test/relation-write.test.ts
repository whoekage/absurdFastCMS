...
test('oneToOne: set then reassign on both sides (displaced partner unlinked)', async () => {
  const schemas = [
    schema({ name: 'person', fields: [{ name: 'name', cmsType: 'string' }], relations: [{ field: 'passport', kind: 'oneToOne', target: 'passport', inverseField: 'holder' }] }),
    schema({ name: 'passport', fields: [{ name: 'code', cmsType: 'string' }] }),
  ];
...
test('oneToOne self-referential: self-link is idempotent; reassign displaces the self-edge', async () => {
  const schemas = [
    schema({ name: 'node', fields: [{ name: 'name', cmsType: 'string' }], relations: [{ field: 'spouse', kind: 'oneToOne', target: 'node', inverseField: 'spouseOf' }] }),
  ];
...
test('unknown relation field error message is "unknown field" (same as a scalar)', async () => {
  const schemas = [schema({ name: 'book', fields: [{ name: 'title', cmsType: 'string' }] })];
  await boot(schemas, async () => {
...
test('FK: a non-existent related id -> 400, NO scalar change, NO link row', async () => {
  const schemas = [
    schema({ name: 'tag', fields: [{ name: 'label', cmsType: 'string' }] }),
    schema({ name: 'book', fields: [{ name: 'title', cmsType: 'string', options: { nullable: false } }], relations: [{ field: 'tags', kind: 'manyToMany', target: 'tag' }] }),
  ];
  let b1!: number;
  await boot(schemas, async () => {
...
test('self-referential manyToOne parent (+ inverse children): set + visible both directions', async () => {
  const schemas = [
    schema({ name: 'category', fields: [{ name: 'slug', cmsType: 'string' }], relations: [{ field: 'parent', kind: 'manyToOne', target: 'category', inverseField: 'children' }] }),
  ];
  let c1!: number, c2!: number;
  await boot(schemas, async () => {
...
test('pure-scalar write is byte-identical: relation-less type, AND a type-with-relations omitting them', async () => {
  const schemas = [
    schema({ name: 'note', fields: [{ name: 'text', cmsType: 'string', options: { nullable: false } }] }),
