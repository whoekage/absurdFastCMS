...
let authed: (p: string, init?: RequestInit) => Promise<Response>; // super-admin cookie ⇒ gated writes pass.
let anon: (p: string, init?: RequestInit) => Promise<Response>; // anonymous ⇒ public reads.
...
before(async () => {
  ...
  const widget: import('../src/db/schema/model.ts').Schema = { id: 'ct_widget', name: 'widget', fields: [{ id: 'f_label', name: 'label', type: 'string', options: { nullable: false } }] };
  const blank: import('../src/db/schema/model.ts').Schema = { id: 'ct_blank', name: 'blank', fields: [] }; // system-fields-only
  const server = await startTestServer(sql, [ARTICLE_SCHEMA, widget, blank], {
  ...
  token = server.token;
  base = server.base;
  authed = server.fetch; // AUTHED super-admin fetch for all gated writes.
  anon = server.anonFetch; // anonymous fetch for public reads.
...
test('POST creates a row, returns 201 + data, and GET sees it (read-after-write)', async () => {
  ...
  const res = await authed(`${base}/article`, j(payload));
  ...
  const got = await anon(`${base}/article/3`);
  assert.equal(got.status, 200);
  assert.deepEqual((await got.json()).data, created);

  // List now has 3 rows.
  const list = await (await anon(`${base}/article?pagination[pageSize]=100`)).json();
  assert.equal(list.data.length, 3);
});

test('PUT partially updates only the provided fields; GET reflects it', async () => {
  const res = await authed(`${base}/article/1`, { method: 'PUT', body: JSON.stringify({ views: 999, status: 'archived' }) });
  ...
  const got = await (await anon(`${base}/article/1`)).json();
  assert.deepEqual(got.data, updated);
});

test('PUT can set a nullable field to null', async () => {
  const res = await authed(`${base}/article/2`, { method: 'PUT', body: JSON.stringify({ title: null }) });
  assert.equal(res.status, 200);
  assert.equal((await res.json()).data.title, null);
  const got = await (await anon(`${base}/article/2`)).json();
  assert.equal(got.data.title, null);
});

test('DELETE removes the row, returns it, and a later GET 404s', async () => {
  const res = await authed(`${base}/article/2`, { method: 'DELETE' });
  assert.equal(res.status, 200);
  assert.equal((await res.json()).data.id, 2);

  assert.equal((await anon(`${base}/article/2`)).status, 404);
  const list = await (await anon(`${base}/article?pagination[pageSize]=100`)).json();
...
test('PUT / DELETE on a nonexistent id -> 404', async () => {
  assert.equal((await authed(`${base}/article/9999`, { method: 'PUT', body: JSON.stringify({ views: 1 }) })).status, 404);
  assert.equal((await authed(`${base}/article/9999`, { method: 'DELETE' })).status, 404);
});
...
test('a valid create still works after the invalid ones (no corrupted state)', async () => {
  const res = await authed(`${base}/article`, j({ body: 'again', status: 'draft', active: false, publishedAt: '2023-01-01T00:00:00.000Z' }));
  assert.equal(res.status, 201);
...
test('PUT with an empty / only-unknown body -> 400 (empty-update guard)', async () => {
  ...
  const empty = await authed(`${base}/article/1`, { method: 'PUT', body: JSON.stringify({}) });
...
test('POST with an over-length string -> a clean 400 (length guard, not a PG 22001 500)', async () => {
  ...
  const res = await authed(`${base}/article`, j({ title: tooLong, body: 'b', status: 'draft', active: true, publishedAt: '2025-01-01T00:00:00.000Z' }));
...
test('system-fields-only module: POST {} -> 201 via DEFAULT VALUES, GET sees it, DELETE removes it', async () => {
  ...
  const res = await authed(`${base}/blank`, j({}));
  ...
  const got = await (await anon(`${base}/blank/${created.id}`)).json();
  assert.deepEqual(got.data, created);

  const del = await authed(`${base}/blank/${created.id}`, { method: 'DELETE' });
  assert.equal(del.status, 200);
  assert.equal((await anon(`${base}/blank/${created.id}`)).status, 404);
});

test('the rebuild path warms the type indexes (no dirty index after a write)', async () => {
  const res = await authed(`${base}/article`, j({ body: 'warm', status: 'draft', active: true, publishedAt: '2026-01-01T00:00:00.000Z' }));
  assert.equal(res.status, 201);
...
test('a write to article does NOT invalidate a sibling type cache (per-type invalidation)', async () => {
  ...
  const w1 = await (await anon(`${base}/widget`)).json();
  ...
  const create = await authed(`${base}/article`, j({ body: 'sib', status: 'draft', active: true, publishedAt: '2024-01-01T00:00:00.000Z' }));
  ...
  const w2 = await (await anon(`${base}/widget`)).json();
