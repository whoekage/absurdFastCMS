...
import { cleanCatalog, schema, rawField } from './helpers.ts';
...
test('to-many populate nests an ARRAY (oneToMany and manyToMany), members in edge-insertion order', async () => {
  ...
  await cleanCatalog(sql);
  const schemasB = [
    schema({ name: 'author', fields: [{ name: 'name', type: 'string' }], relations: [{ field: 'books', kind: 'oneToMany', target: 'book' }] }),
...
test('self-referential populate terminates by the depth cap (no throw, no hang)', async () => {
  const schemas = [
    schema({ name: 'category', fields: [{ name: 'slug', type: 'string' }], relations: [{ field: 'parent', kind: 'manyToOne', target: 'category' }] }),
  ];
  await applySchemas(schemas);
...
test('populate=* expands all declared relations (depth-1); equals explicit naming; relation-less type unchanged + cached', async () => {
  ...
  await cleanCatalog(sql);
  const schemasB = [schema({ name: 'note', fields: [{ name: 'text', type: 'string' }] })];
  await applySchemas(schemasB);
  ...
  const schemas = [
    schema({
      ...
      fields: [
        ...
        { name: 'amount', type: 'decimal', options: { precision: 18, scale: 4 } },
        { name: 'blob', type: 'json' },
      ],
    }),
    schema({ name: 'book', fields: [{ name: 'title', type: 'string' }], relations: [{ field: 'metric', kind: 'manyToOne', target: 'metric' }] }),
  ];
  await applySchemas(schemas);
...
test('inverse cardinality flips: oneToMany owner -> inverse field is a to-one OBJECT/null; manyToMany inverse stays an ARRAY', async () => {
  ...
  const schemasB = [
    schema({ name: 'tag', fields: [{ name: 'label', type: 'string' }] }),
    schema({ name: 'post', fields: [{ name: 'title', type: 'string' }], relations: [{ field: 'tags', kind: 'manyToMany', target: 'tag', inverseField: 'posts' }] }),
  ];
  await applySchemas(schemasB);
