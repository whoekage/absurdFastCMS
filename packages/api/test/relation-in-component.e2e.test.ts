...
const authorType = schema({ name: 'author', fields: [{ name: 'name', type: 'string', options: { nullable: false } }] });
...
test('R0b a top-level `relation` field is rejected by the files-first migrate (component-only kind)', async () => {
  const bad = schema({ name: 'post', fields: [{ name: 'writer', type: 'relation', options: { target: 'author' } }] });
...
test('R1 single relation ref inside a single component: stored inline, read verbatim, populated on read', async () => {
  ...
  const post = schema({ name: 'post', fields: [{ name: 'by', type: 'component', options: { component: 'byline' } }] });
...
test('R2b a wrong-target id that does not exist in the declared target is rejected (400)', async () => {
  const byline = component('byline', [{ name: 'writer', type: 'relation', options: { target: 'author' } }]);
  const tag = schema({ name: 'tag', fields: [{ name: 'slug', type: 'string', options: { nullable: false } }] });
  const post = schema({ name: 'post', fields: [{ name: 'by', type: 'component', options: { component: 'byline' } }] });
...
test('R3 many relation refs inside a component: array stored, populated to an array of objects', async () => {
  ...
  const post = schema({ name: 'post', fields: [{ name: 'credits', type: 'component', options: { component: 'credits' } }] });
  const solo = schema({ name: 'solo', fields: [{ name: 'one', type: 'component', options: { component: 'one' } }] });
...
test('R4 relation refs inside a repeatable component and a dynamic-zone block populate correctly', async () => {
  ...
  const rep = schema({ name: 'rep', fields: [{ name: 'rows', type: 'component-repeatable', options: { component: 'row' } }] });
  const zoned = schema({ name: 'zoned', fields: [{ name: 'body', type: 'dynamiczone', options: { components: ['authorBlock'] } }] });
...
test('R6 a DRAFT target resolves to null on read (default published-only visibility)', async () => {
  ...
  const author = schema({ name: 'author', fields: [{ name: 'name', type: 'string', options: { nullable: false } }], draftPublish: true });
  const byline = component('byline', [{ name: 'writer', type: 'relation', options: { target: 'author' } }]);
  const post = schema({ name: 'post', fields: [{ name: 'by', type: 'component', options: { component: 'byline' } }] });
...
test('R7 an i18n target resolves in the default locale on read', async () => {
  const author = schema({ name: 'author', fields: [{ name: 'name', type: 'string', options: { nullable: false }, localized: true }], i18n: true });
  const byline = component('byline', [{ name: 'writer', type: 'relation', options: { target: 'author' } }]);
  const post = schema({ name: 'post', fields: [{ name: 'by', type: 'component', options: { component: 'byline' } }] });
  await boot([author, post], [byline]);
...
test('R9 a component without a relation-ref field reads identically (no relation populate effect)', async () => {
  ...
  const page = schema({ name: 'page', fields: [{ name: 'seo', type: 'component', options: { component: 'seo' } }] });
  await boot([page], [seo]);
...
test('R10 a non-component module is byte-identical (relation-ref machinery never runs)', async () => {
  const plainType = schema({ name: 'plain', fields: [{ name: 'title', type: 'string', options: { nullable: false } }] });
