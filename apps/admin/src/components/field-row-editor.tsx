...
interface FieldRowEditorProps {
  ...
  /** When the module is i18n, render a per-field Localized/Shared toggle (hidden otherwise). */
  ...
  return (
    <div className="space-y-3 rounded-md border p-4">
      <div className="flex items-end gap-3">
        ...
        {onRemove && (
          <Button type="button" variant="ghost" size="icon" title="Remove field" onClick={onRemove}>
      ...
      {meta.multiple && (
        <label
          ...
          title="Multiple: this media field holds an ARRAY of assets. Unchecked = a single asset reference."
        >
          <input
            type="checkbox"
      ...
      <div className="flex flex-wrap items-end gap-4">
        ...
        {i18n && (
          <label
            className="flex items-center gap-2 text-sm"
            title="Localized: a per-locale value. Unchecked = shared across all locale variants of a document."
          >
            <input
              type="checkbox"
              className="h-4 w-4 rounded border-input"
...
function EnumValuesEditor({
  ...
  return (
    <div className="space-y-2">
      <Label>Values</Label>
      {values.length === 0 && <p className="text-xs text-muted-foreground">Add at least one enum value.</p>}
      {values.map((value, index) => (
        <div key={`${draftKey}-enum-${index}`} className="flex items-center gap-2">
          ...
          />
          <Button type="button" variant="ghost" size="icon" title="Remove value" onClick={() => removeAt(index)}>
            <X className="h-4 w-4" />
          </Button>
        </div>
      ))}
