...
before(async () => {
  ...
  await withCatalogWrite(sql, async () => {
    if (await getContentType(sql, 'sec_article')) await dropContentType(sql, 'sec_article');
    ...
    if (await getContentType(sql, 'sec_doc')) await dropContentType(sql, 'sec_doc');
    await createContentType(sql, { apiId: 'sec_doc', fields: [{ name: 'blob', cmsType: 'json', options: { nullable: false } }] });
  });
  await sql`INSERT INTO ct_sec_article (title, body, status, views, rating, active, "publishedAt")
...
after(async () => {
  ...
  await withCatalogWrite(sql, async () => {
    await dropContentType(sql, 'sec_article');
    await dropContentType(sql, 'sec_doc');
  });
  await sql.end();
});
...
test('injection via field name -> 400 unknown field, before any SQL; ct_article survives', async () => {
  const body = {
    '"; DROP TABLE ct_sec_article;--': 1,
    body: 'x',
    status: 'draft',
    active: true,
    publishedAt: '2022-01-01T00:00:00.000Z',
  };
  const res = await fetch(`${base}/sec_article`, { method: 'POST', body: JSON.stringify(body) });
  assert.equal(res.status, 400);
  assert.match((await res.json()).error, /unknown field/);
  assert.equal(await tableExists('ct_sec_article'), true);
});
...
test('mass-assignment: id / created_at / updated_at are rejected (400)', async () => {
  ...
  for (const key of ['id', 'created_at', 'updated_at']) {
    const res = await fetch(`${base}/sec_article`, { method: 'POST', body: JSON.stringify({ ...ok, [key]: key === 'id' ? 99 : '2000-01-01T00:00:00.000Z' }) });
...
test('jsonb nested big int + key order survive load+respond byte-exact (the ::text path)', async () => {
  ...
  await sql.unsafe(`INSERT INTO ct_sec_doc (blob) VALUES ('{"big": ${bigInt}, "z": 1, "a": 2}'::jsonb)`);
  // Reload the engine so the directly-inserted row is visible.
  const store = new PostgresStore(sql);
  const { engine } = await withCatalogRead(sql, () => store.loadWithRegistry());
  const buf = engine.respondById('sec_doc', 1)!.toString('utf8');
