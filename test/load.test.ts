...
async function cleanCatalog(): Promise<void> {
  await withCatalogWrite(sql, async () => {
    const tables = await sql<{ table_name: string }[]>`SELECT table_name FROM information_schema.tables WHERE table_schema = 'public' AND table_name LIKE 'ct\\_ld\\_%'`;
    for (const { table_name } of tables) await sql.unsafe(`DROP TABLE IF EXISTS "${table_name}" CASCADE`);
    await sql`DELETE FROM content_types WHERE api_id LIKE 'ld\\_%'`;
  });
}
...
test('multi-type load with i64/decimal/json byte-exact round-trip; warm once; empty type 0 rows', async () => {
  await withCatalogWrite(sql, async () => {
    ...
    await createContentType(sql, { apiId: 'ld_empty', fields: [{ name: 'x', cmsType: 'integer', options: { nullable: true } }] });
  });

  // jsonb with a nested integer > 2^53 and out-of-order keys.
  const bigInt = 9007199254740993; // 2^53 + 1
  await sql.unsafe(
    `INSERT INTO ct_ld_note (big, price, meta) VALUES ('9223372036854775807', '12345.67', '{"big": ${bigInt}, "z": 1, "a": 2}'::jsonb)`,
  );
  // A NULL in each nullable column, plus a jsonb literal 'null' value (distinct from SQL NULL).
  await sql.unsafe(`INSERT INTO ct_ld_note (big, price, meta) VALUES (NULL, NULL, NULL)`);
  await sql.unsafe(`INSERT INTO ct_ld_note (big, price, meta) VALUES ('1', '0.01', 'null'::jsonb)`);

  const engine = await withCatalogRead(sql, async () => {
    const registry = await Registry.build(sql);
    return buildEngine(sql, registry);
  });

  assert.equal(engine.rowCount('ld_note'), 3);
  assert.equal(engine.rowCount('ld_empty'), 0);
  assert.equal(engine.respondById('ld_empty', 1), null);

  // warm once: no dirty index after load.
  assert.equal(engine.table('ld_note').hasDirtyIndex(), false);
  ...
  const b1 = engine.respondById('ld_note', 1)!;
  ...
  const o2 = JSON.parse(engine.respondById('ld_note', 2)!.toString('utf8'));
  ...
  const b3 = engine.respondById('ld_note', 3)!;
